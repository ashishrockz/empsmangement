...
const LeaveRequestAproval = ({ route }: { route: any; }) => {
  ...
  const fetchLeaveRequest = async () => {
    try {
      const response = await axios.get(`https://emps-jade.vercel.app/api/leave/details/${leaveId}`);
  ...
  const handleApproval = async (status: string) => {
    try {
      const response = await axios.put(`https://emps-jade.vercel.app/api/leave/${leaveId}`, {
        approverRole,
        status,
      });
